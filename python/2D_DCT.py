...
def dct_2d_q88(img_u8: np.ndarray) -> np.ndarray:
    """ Q8.8 2D DCT (行方向 + 列方向) """
    tmp = np.array([dct_1d_u8(row) for row in img_u8])
    return np.array([dct_1d_u8(tmp[:, i]) for i in range(8)]).T
...
def dct_2d_fftpack(img_u8: np.ndarray) -> np.ndarray:
    """ fftpack による 2D DCT """
    return dct(dct(img_u8.astype(np.float64), axis=0, norm='ortho'), axis=1, norm='ortho')
