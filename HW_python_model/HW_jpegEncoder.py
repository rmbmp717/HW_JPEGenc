...
def dct_2d_manual(img_u8: numpy.ndarray, level_shift_value) -> numpy.ndarray:
    ...
    # 各行に対して1D DCT
    temp = numpy.array([HW_dct_model.dct_1d_manual(row) for row in img_f])
    # 各列に対して1D DCTを適用して転置して元の形に戻す
    return numpy.array([HW_dct_model.dct_1d_manual(temp[:, i]) for i in range(temp.shape[1])]).T

def dct_2d_q88(img_u8: numpy.ndarray, level_shift_value) -> numpy.ndarray:
    img_f = img_u8.astype(numpy.float64) - level_shift_value
    tmp = numpy.apply_along_axis(HW_dct_model.dct_1d_q88, axis=1, arr=img_f)  # 行ごとに DCT
    return numpy.apply_along_axis(HW_dct_model.dct_1d_q88, axis=0, arr=tmp)   # 列ごとに DCT
...
def main():
    ...
    for y in range(0, imageHeight, 8):
        for x in range(0, imageWidth, 8):
            ...
            #yDctMatrix = dct_2d_fftpack(yImageMatrix[y:y + 8, x:x + 8], 0)
            #yDctMatrix = dct_2d_manual(yImageMatrix[y:y + 8, x:x + 8], 0)
            yDctMatrix = dct_2d_q88(yImageMatrix[y:y + 8, x:x + 8], 0)

            #uDctMatrix = fftpack.dct(fftpack.dct(uImageMatrix[y:y + 8, x:x + 8], norm='ortho').T, norm='ortho').T
            #uDctMatrix = dct_2d_fftpack(uImageMatrix[y:y + 8, x:x + 8], 0)
            #uDctMatrix = dct_2d_manual(uImageMatrix[y:y + 8, x:x + 8], 0)
            uDctMatrix = dct_2d_q88(uImageMatrix[y:y + 8, x:x + 8], 0)
            
            #vDctMatrix = fftpack.dct(fftpack.dct(vImageMatrix[y:y + 8, x:x + 8], norm='ortho').T, norm='ortho').T
            #vDctMatrix = dct_2d_fftpack(vImageMatrix[y:y + 8, x:x + 8], 0)
            ...
            if(blockSum<=8):
                ...
                #print('uDctMatrix_fft:\n',uDctMatrix_fft.astype(int32))
                print('uDctMatrix:\n',uDctMatrix.astype(int32))
                #print('uDctMatrix_q88:\n',uDctMatrix_q88.astype(int32))
                #print('vDctMatrix_fft:\n',vDctMatrix_fft.astype(int32))
                print('vDctMatrix:\n',vDctMatrix.astype(int32))
                #print('vDctMatrix_q88:\n',vDctMatrix_q88.astype(int32))
            ...
            if(DEBUG_MODE==1):
                print('yQuantMatrix:\n',yQuantMatrix.astype(int32))
                print('uQuantMatrix:\n',uQuantMatrix.astype(int32))
                print('vQuantMatrix:\n',vQuantMatrix.astype(int32))
            ...
            sosBitStream.append(huffmanEncode.encodeDCToBoolList(dyDC[blockNum],1, DEBUG_MODE))
            # encode yAC
            if (DEBUG_MODE == 1):
            ...
            sosBitStream.append(huffmanEncode.encodeDCToBoolList(dvDC[blockNum],0, DEBUG_MODE))
            # encode uAC
            if (DEBUG_MODE == 1):
